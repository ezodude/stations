...
class PodcastTest < Test::Unit::TestCase
  ...
  def test_builds_correctly
    testee = Podcast.build(AUDIO_URI, TITLE, SUMMARY, DURATION, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE)
    
    expected = Podcast.new(FAKE_PODCAST_ID, AUDIO_URI, TITLE, SUMMARY, DURATION, SOURCE_URI, TAGS, \
  ...
  def test_throws_exception_when_created_with_blank_audio_uri
    assert_raises(RuntimeError) { Podcast.build(nil, TITLE, SUMMARY, DURATION, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) }
    assert_raises(RuntimeError) { Podcast.build('', TITLE, SUMMARY, DURATION, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) }
  end
  ...
  def test_throws_exception_when_created_for_non_mp3_formatted_audio
    ...
    assert_raises(RuntimeError) do 
      Podcast.build(invalid_audio_uri, TITLE, SUMMARY, DURATION, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) 
    end
  end
  
  def test_throws_exception_when_created_with_blank_title
    assert_raises(RuntimeError) { Podcast.build(AUDIO_URI, nil, SUMMARY, DURATION, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) }
    assert_raises(RuntimeError) { Podcast.build(AUDIO_URI, '', SUMMARY, DURATION, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) }
  end
  
  def test_throws_exception_when_created_with_blank_duration
    assert_raises(RuntimeError) { Podcast.build(AUDIO_URI, TITLE, SUMMARY, nil, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) }
    assert_raises(RuntimeError) { Podcast.build(AUDIO_URI, TITLE, SUMMARY, 0, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) }
  end
  
  def test_throws_exception_when_created_with_duration_less_than_9_minutes
    assert_raises(RuntimeError) { Podcast.build(AUDIO_URI, TITLE, SUMMARY, 8, SOURCE_URI, TAGS, PUBLISHED_AT, FILE_SIZE) }
  end
end