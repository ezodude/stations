...
class StationsTest < Test::Unit::TestCase
  ...
  def test_retrieves_a_listeners_recently_listened_to_programmes_during_inactive_listening
    browser = setup_mock_browser
    time = Time.now.utc
    ...
    fake_listener.should_receive(:recent_programmes_indexed_by_station).with(false).returns(recent_progs)
    flexmock(TrackedListener).should_receive(:first).with(:id => 'some-id').returns(fake_listener)
    
    browser.get '/listeners/some-id/recent_programmes.json', {'listening_state' => 'inactive'}
    assert(browser.last_response.ok?)
